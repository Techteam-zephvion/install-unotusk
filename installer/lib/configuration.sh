...
run_config_wizard() {
  ...
  local gum_bin=""
  if [ "${UNATTENDED:-}" != "true" ] && [ "${UNOTUSK_NO_GUM:-}" != "true" ]; then
...
def ask(header, default='', secret=False):
    ...
    if GUM_BIN:
        ...
        try:
            # Only capture stdout (gum's final answer). Leave stderr
            # connected to the real terminal instead of a pipe — gum's own
            # warnings/errors (e.g. TTY problems) were previously being
            # silently swallowed, which is exactly why a broken render
            # showed nothing at all instead of a diagnosable message.
...
#
# CLIENT_CRT/CLIENT_KEY point at the bootstrap UCA client cert
# certificates.sh generates (US/certs/uca-client/), signed by the same
# mesh CA US already trusts unconditionally — this is what lets
# ExchangeRef's mTLS channel even open. It's a shared per-deployment
# identity, not per-employee: real per-employee certs (POST
# /client-cert/issue, session-gated — see US/src/employee_pki.rs) are
# only obtainable after a session already exists, so they can't be what
# bootstraps the very first login. UCA swapping to a per-employee cert
# post-login for other mTLS calls is a separate, not-yet-built feature.
#
# PLATFORM_BASE_URL overrides UCA's compiled-in default
# (https://platform.unotusk.com) — that's a placeholder domain, not the
# real cloud Platform deployment. Without this override, project-list
# fetch (POST-login) fails at the transport layer with no obvious cause
# ("Can't reach the server"), even though login itself already succeeded.
#
# UPS_BASE_URL is confusingly named but targets AI-PIE (port 8000), not
# UPS — UCA's "Ask"/spec-generation calls hit AI-PIE directly, never UPS.
# Pointing this at UPS's own port (8443, its ADMIN mTLS pairing listener)
# sends every Ask call into a listener that mandates a client cert UCA
# was never given, failing at the TLS handshake ("Client auth requested
# but no cert/sigscheme available") with no indication the URL itself
# was wrong.
write_client_env() {
  ...
  local client_crt_path="$client_env_dir/uca-client.crt"
  local client_key_path="$client_env_dir/uca-client.key"
  if [ -f "$INSTALL_DIR/US/certs/uca-client/cert.pem" ]; then
    cp "$INSTALL_DIR/US/certs/uca-client/cert.pem" "$client_crt_path"
    cp "$INSTALL_DIR/US/certs/uca-client/key.pem" "$client_key_path"
    chmod 644 "$client_crt_path" "$client_key_path"
  fi

  # AI-PIE dev mTLS identity ("aipie-dev-client", certificates.sh) — what
  # UPS_CA_CERT/UPS_CLIENT_CRT/UPS_CLIENT_KEY below actually authenticate
  # against, since UPS_BASE_URL targets AI-PIE (see comment above).
  local ups_ca_path="$client_env_dir/ai-pie-ca.crt"
  local ups_client_crt_path="$client_env_dir/ai-pie-client.crt"
  local ups_client_key_path="$client_env_dir/ai-pie-client.key"
  if [ -f "$INSTALL_DIR/AI-PIE/certs/dev/ca.pem" ]; then
    cp "$INSTALL_DIR/AI-PIE/certs/dev/ca.pem" "$ups_ca_path"
    cp "$INSTALL_DIR/AI-PIE/certs/dev/client.pem" "$ups_client_crt_path"
    cp "$INSTALL_DIR/AI-PIE/certs/dev/client.key" "$ups_client_key_path"
    chmod 644 "$ups_ca_path" "$ups_client_crt_path" "$ups_client_key_path"
  fi
...
UPS_BASE_URL=https://${HOSTNAME}:8000
CA_CERT=$client_ca_path
CLIENT_CRT=$client_crt_path
CLIENT_KEY=$client_key_path
PLATFORM_BASE_URL=https://unotusk-platform-service.onrender.com
UPS_CA_CERT=$ups_ca_path
UPS_CLIENT_CRT=$ups_client_crt_path
UPS_CLIENT_KEY=$ups_client_key_path
EOF
